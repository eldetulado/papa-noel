...
import com.codebear.metodos.databinding.ActivityReposoBinding
...
class ReposoActivity : AppCompatActivity() {

    private val data = mapOf(
        Pair("MAQUINA", "--- No aplica ---"),
        Pair("TIPO DE OPERACIÓN", "--- No aplica ---"),
        Pair("CAPACIDAD DE LA OPERACIÓN", "12,20 Kg de masa"),
        Pair("CAPACIDAD DE MESA DE REPOSO", "20,00 Kg de masa"),
        Pair("MERMAS", "0,000%"),
        Pair("DEFECTUSOS", "0,000%"),
        Pair("SET-UP", "0,25 min/lote"),
        Pair("DISTANCIA A OPERACIÓN POSTERIOR", "2,10 m"),
    )

    private lateinit var binding: ActivityReposoBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityReposoBinding.inflate(layoutInflater)
        setContentView(binding.root)

        title = "Operación de reposo"
    ...
    private fun loadPrefs() {
        val data = PrefsUtil.getData(this, Step.REPOSO)
        data?.let {
